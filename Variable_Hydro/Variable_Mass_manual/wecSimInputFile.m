...
simu.simMechanicsFile = 'sphereNormal.slx';    % Specify Simulink Model File
...
% Sphere
% body(1) = bodyClass(h5Files); % bodyClass('hydroData/draft1.h5'); %
body(1) = bodyClass(h5File);
body(1).mass = massVal;
body(1).inertia = inertiaVal;
body(1).initial.displacement = [0, 0, zDisp];
...
% body(1).variableHydro.mass = massVals; % 'equilibrium' for each sphere size. If not set, equilibrium will also be assumed.
